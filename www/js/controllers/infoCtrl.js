...
angular.module('starter')
    ...
    .controller('TimeCtrl', function($scope, $q, $filter, TimeAttendanceSQLite, SyncService, $ionicPlatform, APIService, $rootScope, $cordovaNetwork, $ionicPopup, $ionicModal, $cordovaDevice) {
                ...
                TimeReportSQLite.GetTimeReportsBySelectedMonthYear(selectedVal).then(function(response){
                    ...
                    for (var i = 0; i <= data.length - 1; i++) {
                        $scope.timeReports.push({
                            WORKDATE:GetThaiDateWithWeekdayByDate($filter,data[i].WORKDATE), //GetThaiDateByDate($filter,data[i].WORKDATE),
